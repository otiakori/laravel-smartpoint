    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="mt-2 text-gray-600">Modify invoice details and items</p>
            </div>
            <div class="flex items-center space-x-3">
                <a href="{{ route('invoices.show', $invoice) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-lg text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-smartpoint-red transition-colors">
        ...
        <form action="{{ route('invoices.update', $invoice) }}" method="POST" id="invoiceForm">
            ...
            <div class="p-6 border-b border-gray-200">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        <input type="date" name="invoice_date" id="invoice_date" value="{{ old('invoice_date', $invoice->invoice_date->format('Y-m-d')) }}" class="block w-full border-gray-300 rounded-lg focus:ring-smartpoint-red focus:border-smartpoint-red">
                        @error('invoice_date')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="due_date" id="due_date" value="{{ old('due_date', $invoice->due_date ? $invoice->due_date->format('Y-m-d') : '') }}" class="block w-full border-gray-300 rounded-lg focus:ring-smartpoint-red focus:border-smartpoint-red">
                        @error('due_date')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="reference_number" id="reference_number" value="{{ old('reference_number', $invoice->reference_number) }}" placeholder="PO-12345" class="block w-full border-gray-300 rounded-lg focus:ring-smartpoint-red focus:border-smartpoint-red">
                        ...
                        <div class="item-row bg-gray-50 rounded-lg p-4 border border-gray-200">
                            <div class="grid grid-cols-1 md:grid-cols-6 gap-4">
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <input type="text" name="items[{{ $index }}][item_name]" value="{{ $item->item_name }}" class="item-name block w-full border-gray-300 rounded-lg focus:ring-smartpoint-red focus:border-smartpoint-red" required>
                                </div>

                                <!-- Quantity -->
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Qty</label>
                                    <input type="number" name="items[{{ $index }}][quantity]" value="{{ $item->quantity }}" class="item-quantity block w-full border-gray-300 rounded-lg focus:ring-smartpoint-red focus:border-smartpoint-red" min="1" required>
                                </div>

                                <!-- Unit Price -->
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Price</label>
                                    <input type="number" name="items[{{ $index }}][unit_price]" value="{{ $item->unit_price }}" class="item-price block w-full border-gray-300 rounded-lg focus:ring-smartpoint-red focus:border-smartpoint-red" step="0.01" min="0" required>
                                </div>

                                <!-- Tax Rate -->
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Tax %</label>
                                    <input type="number" name="items[{{ $index }}][tax_rate]" value="{{ $item->tax_rate }}" class="item-tax block w-full border-gray-300 rounded-lg focus:ring-smartpoint-red focus:border-smartpoint-red" step="0.01" min="0" max="100">
                                </div>

                                <!-- Discount -->
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Discount</label>
                                    <input type="number" name="items[{{ $index }}][discount_amount]" value="{{ $item->discount_amount }}" class="item-discount block w-full border-gray-300 rounded-lg focus:ring-smartpoint-red focus:border-smartpoint-red" step="0.01" min="0">
                                </div>

                                <!-- Total -->
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Total</label>
                                    <input type="text" value="{{ number_format($item->total_amount, 2) }}" class="item-total block w-full border-gray-300 rounded-lg bg-gray-100" readonly>
                                </div>
            ...
            <div class="p-6 border-b border-gray-200">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <textarea name="notes" id="notes" rows="3" placeholder="Additional notes for the customer..." class="block w-full border-gray-300 rounded-lg focus:ring-smartpoint-red focus:border-smartpoint-red">{{ old('notes', $invoice->notes) }}</textarea>
                        @error('notes')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="terms_conditions" id="terms_conditions" rows="3" placeholder="Payment terms and conditions..." class="block w-full border-gray-300 rounded-lg focus:ring-smartpoint-red focus:border-smartpoint-red">{{ old('terms_conditions', $invoice->terms_conditions) }}</textarea>
            ...
            <div class="p-6">
                <div class="flex justify-between items-center">
                    <div class="text-lg font-medium text-gray-900">Total Amount: <span id="totalAmount" class="text-smartpoint-red">{{ formatCurrency($invoice->total_amount) }}</span></div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function addItem() {
        ...
        setupItemCalculations(itemDiv);
        invoiceItems.appendChild(itemDiv);
        itemIndex++;
    }

    function setupItemCalculations(itemDiv) {
